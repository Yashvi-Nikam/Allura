...
const RECOMMENDATION_MODES: {
...
}[] = [
  {
    ...
    description: 'Build looks using pieces you already own.',
  },
  {
    value: 'scratch',
    title: 'Create from scratch',
    description: 'Create the complete look independently of your wardrobe.',
  },
  {
    value: 'blend',
    title: 'Blend both',
    description: 'Use your wardrobe where it works and suggest new pieces where useful.',
  },
];
...
export default function Context() {
  ...
  const [recommendationMode, setRecommendationMode] =
    useState<RecommendationMode>('wardrobe');
  ...
  const handleSubmit = async () => {
    try {
       ...
       * There is NO hardcoded demo user here.
       */
      const {
        data: { user },
        error: userError,
      ...
      /*
       * Keep the free-text anchor as an additional preference
       * for now.
       *
       * Later this can become a real wardrobe item picker
       * which sends anchor_item_id.
       */
      ...
      /*
       * Send the authenticated user's actual UUID.
       */
      ...
      /*
       * Move to result screen after successful generation.
       */
      router.push({
        pathname: '/result',
        params: {
  ...
  if (loading) {
    return (
      <LoadingState
        message={
          recommendationMode === 'scratch'
            ? 'Allura is creating your looks...'
            : recommendationMode === 'blend'
              ? 'Allura is blending your wardrobe with new ideas...'
              : 'Allura is curating your looks...'
        }
      />
    );
  }
            ...
            {RECOMMENDATION_MODES.map((mode) => {
              ...
              return (
                <TouchableOpacity
                  ...
                  onPress={() =>
                    setRecommendationMode(mode.value)
                  }
                  activeOpacity={0.75}
                >
                  <Text
...
const styles = StyleSheet.create({
  ...
  modeCardSelected: {
    backgroundColor: 'rgba(201,171,133,0.12)',
