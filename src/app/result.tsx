...
export default function Results() {
  ...
  const { saveOutfit, addFeedback } = useRecommendations(); // Get addFeedback
  ...
  const handleSave = async () => {
    ...
    await addFeedback(outfit.id, 'liked', outfit.why_this_works);
  ...
  const handleSkip = async () => {
    ...
    await addFeedback(outfit.id, 'skipped');
    setCurrent(c => Math.min(outfits.length - 1, c + 1));
  };
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.actionsRow}>
          {/* "Skip" button -> now sends feedback */}
          <TouchableOpacity
          ...
          {/* "Save" button -> sends positive feedback */}
          <TouchableOpacity
